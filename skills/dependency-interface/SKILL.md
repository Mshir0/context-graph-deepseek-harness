...
For a code change, retain the prior Dependency Skill JSON, call `dependency_detect_changes` with it and the changed project-relative Python files, then review added and removed facts. Dynamic facts have low confidence and must not be represented as confirmed dependencies.
