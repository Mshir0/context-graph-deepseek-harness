...
const NODE_TYPES = ['code_module', 'requirement', 'task', 'constraint', 'decision', 'interface', 'documentation', 'conversation', 'artifact', 'test', 'issue', 'note', 'project_rule'];
...
function Inspector({ graph, selected, updateNode, updateEdge, remove }) {
  ...
  return h('section', { className: 'cg-inspector', style: position ? { left: position.left, top: position.top, right: 'auto', bottom: 'auto', width: 300 } : undefined },
    ...
    h('div', { className: 'cg-form' }, selected.kind === 'node' ? [
      ...
      h('label', { key: 'lc' }, '内容'),
      h('textarea', { key: 'c', value: item.content || '', onChange: event => updateNode({ content: event.target.value }) }),
      h('label', { key: 'lp' }, '模块路径'),
...
function GraphPanel({ sessionId, projectPath, sendPrompt, setTarget }) {
  ...
  const addNode = useCallback(() => {
    ...
    const node = { id, type: 'note', title: '新建上下文', label: '新建上下文', content: '', source: 'user', priority: 'normal', status: 'active', mode: 'MANUAL', metadata: {}, x: Math.max(40, (120 - view.x) / view.zoom), y: Math.max(40, (100 - view.y) / view.zoom) };
  ...
  useEffect(() => {
    const keydown = event => {
      ...
      else if (event.key === 'Escape') { setGesture(null); setSelected(null); setInspected(null); setHelp(false); setPreviewData(null); }
    };
    window.addEventListener('keydown', keydown);
    return () => window.removeEventListener('keydown', keydown);
  }, [fit, layout, remove, save, submit]);

  const visibleNodes = useMemo(() => (graph?.nodes || []).filter(node => (typeFilter === 'all' || (node.type || 'code_module') === typeFilter) && (!search.trim() || `${node.title || node.label || node.id} ${node.content || ''} ${node.path || ''}`.toLowerCase().includes(search.trim().toLowerCase()))), [graph, search, typeFilter]);
