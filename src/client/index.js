...
.cg-context-button{border:0;background:transparent;color:#374151;font:inherit;font-weight:500;padding:5px 7px;border-radius:5px;cursor:pointer;white-space:nowrap}.cg-context-button:hover,.cg-context-button[aria-expanded=true]{background:#f3f4f6;color:#111827}.cg-context-menu{position:fixed;z-index:50;width:min(324px,calc(100vw - 24px));padding:12px;background:#fff;color:#1f2937;border:1px solid #e5e7eb;border-radius:8px;box-shadow:0 12px 30px rgb(15 23 42 / 14%)}.cg-context-menu label{display:block;margin:0 0 5px;font-size:12px;font-weight:500;color:#6b7280}.cg-context-menu select,.cg-context-menu textarea{width:100%;border:1px solid #d1d5db;border-radius:6px;background:#fff;color:#1f2937;padding:8px 9px;font:inherit;outline:none}.cg-context-menu select{height:36px}.cg-context-menu textarea{display:block;min-height:96px;resize:vertical;line-height:1.5}.cg-context-menu select:focus,.cg-context-menu textarea:focus{border-color:#60a5fa;box-shadow:0 0 0 3px #dbeafe}.cg-context-target{display:flex;align-items:center;min-height:28px;margin:10px 0 12px;padding:5px 8px;border-radius:5px;background:#f8fafc;color:#64748b;font-size:12px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.cg-context-menu button{margin-top:10px;border:0;border-radius:6px;padding:7px 10px;background:#2563eb;color:#fff;font:inherit;font-weight:500;cursor:pointer}.cg-context-menu button:hover{background:#1d4ed8}
body[data-ds-dark-theme] .cg-root{--cg-line:#34343a;--cg-muted:#a1a1aa;--cg-text:#f4f4f5;--cg-bg:#18181b;--cg-soft:#27272a}body[data-ds-dark-theme] .cg-canvas-wrap{background-color:#1c1c1f;background-image:radial-gradient(#3f3f46 1px,transparent 1px)}body[data-ds-dark-theme] .cg-node-box,body[data-ds-dark-theme] .cg-form input,body[data-ds-dark-theme] .cg-form select,body[data-ds-dark-theme] .cg-compose textarea,body[data-ds-dark-theme] .cg-compose select{fill:#202023;background:#202023;color:#f4f4f5;border-color:#3f3f46}body[data-ds-dark-theme] .cg-node-head{fill:#2b2b30}body[data-ds-dark-theme] .cg-node-title{fill:#f4f4f5}body[data-ds-dark-theme] .cg-node-meta{fill:#a1a1aa}body[data-ds-dark-theme] .cg-tools,body[data-ds-dark-theme] .cg-status,body[data-ds-dark-theme] .cg-inspector,body[data-ds-dark-theme] .cg-help{background:#202023eb;border-color:#3f3f46}body[data-ds-dark-theme] .cg-icon{color:#d4d4d8}body[data-ds-dark-theme] .cg-context-button{color:#d4d4d8}body[data-ds-dark-theme] .cg-context-button:hover,body[data-ds-dark-theme] .cg-context-button[aria-expanded=true]{background:#2b2b30;color:#fff}body[data-ds-dark-theme] .cg-context-menu{background:#202023;color:#f4f4f5;border-color:#3f3f46;box-shadow:0 12px 30px #0006}body[data-ds-dark-theme] .cg-context-menu label{color:#a1a1aa}body[data-ds-dark-theme] .cg-context-menu select,body[data-ds-dark-theme] .cg-context-menu textarea{background:#27272a;color:#f4f4f5;border-color:#48484f}body[data-ds-dark-theme] .cg-context-target{background:#29292e;color:#c4c4cc}
`;
...
function ContextCommand({ sessionId, inputActions, targetStore }) {
  ...
  return h('div', null, h('button', { ref: buttonRef, type: 'button', className: 'cg-context-button', title: '上下文任务', 'aria-expanded': open, onClick: () => setOpen(value => !value) }, '上下文'), open && rect ? h('section', { className: 'cg-context-menu', role: 'dialog', 'aria-label': '上下文任务', style: { left: Math.min(rect.left, window.innerWidth - 336), bottom: window.innerHeight - rect.top + 8 } }, h('label', null, '任务类型'), h('select', { value: taskType, onChange: event => setTaskType(event.target.value) }, TASKS.map(([key, label]) => h('option', { key, value: key }, label))), h('div', { className: 'cg-context-target', title: target || '自动识别目标模块' }, target ? `目标模块：${target}` : '目标模块：自动识别'), h('label', null, '任务内容'), h('textarea', { value: task, placeholder: '描述需要完成的工作', onChange: event => setTask(event.target.value) }), h('button', { type: 'button', onClick: addToDraft }, '添加到输入框')) : null);
}
